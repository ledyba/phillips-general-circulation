...
　130日目まで実行すると、以下の図のようになります（そして一旦停止するようになっています）。線形で温めている効果が出て、上部（北）では冷たく、下部（南）では暖かくなっています。青と赤はそれぞれその地点での温度をしめしていて、茶色からライムイエローの線は大気圧が500hPaになる等高線で、ライムイエローが等高線が低い場所、茶色が高い場所を示しています。
...
　この130日目までの計算にはあとで加えるようなランダムなノイズは入れていないため、何度実行しても必ず同じ結果になります。ですから、Philipsの元の結果と合致してうまく動いているかどうか調べることができます。
